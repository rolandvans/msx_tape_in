...
#include <math.h>
// checksum
#include <CRC32.h>  // CRC32 library by Christopher Baker, MIT license
/* For SDcard
  #include <SPI.h>
  #include <SD.h>
...
uint8_t N, cycle025, cycle075, cycle100, cycle200, cycle050, printpos, fileopen;
uint16_t signalthreshold, byteswritten;
uint32_t crcval;
...
void savedata(uint8_t option) {
  ...
  if (writeflag) {
    for (i = 0; i < DECODEBUFSIZE / 2; i++) {
      printserial(decodebuf[i + bufpart * (DECODEBUFSIZE / 2)]);
    }
    writeflag = 0;
  }
  if (option) {
